...
This probability is conceptually called **LZPGEN** (LZ-generation probability). In the Python API, it is computed using the `pgen()` method.
...
log_p = graph.pgen(seq) # (1)
print(f"log P(gen) = {log_p:.2f}")
```

1. Returns the **natural log** of the probability by default. The raw probability would be $e^{\text{log\_p}}$. Pass `log=False` to get the raw probability.
...
scores = graph.pgen([common_seq, rare_seq, random_seq])
...
log_p = graph.pgen("CASSLGIRRT")

# Raw probability (use with caution)
p = graph.pgen("CASSLGIRRT", log=False)
print(f"log P = {log_p:.4f}  →  P = {p:.2e}")
```

### Batch scoring

When you need to score hundreds or thousands of sequences, pass them as a list. This is much faster than calling `pgen` in a loop because the C backend processes them in batch:

```python title="Batch scoring 5,000 sequences"
import numpy as np

# Score all training sequences
all_scores = graph.pgen(seqs) # (1)
...
1. Pass a `list[str]` to score all sequences in a single call. Returns a `np.ndarray` of the same length. ~10,000 sequences/sec on typical hardware.
...
This is equivalent to checking `graph.pgen(seq) > -690` but reads more naturally in conditionals.

---

## Anomaly scoring (FlashBackGraph + SCALE)

If you are using a `FlashBackGraph`, you can score how "anomalous" a sequence is
with **SCALE**, the self-calibrated anomaly score. While `pgen` tells you the
probability of a sequence, SCALE turns it into a length-invariant surprise
score by calibrating against the graph's own output.
...
cal = fb_graph.calibrate_scale(seed=0)             # self-calibrate once
anomaly_score = fb_graph.scale_score("CASSLEPSGGTDTQYF", cal)  # higher = more anomalous
print(f"SCALE score: {anomaly_score:.2f}")
```

See the [FlashBack anomaly-detection tutorial](flashback-anomaly.md) for the
full workflow, including choosing a flag threshold.
...
scores = population_graph.pgen(individual_seqs)
