/**
 * @file fbg_posterior.c
 * @brief Dirichlet-Multinomial posterior FlashBackGrammar.
 *
 * Reference: fbg_prototype.py `posterior`.
 *
 ...
 *
 * Mutation hygiene: decomposition uses stack LZGFbgStep[], so prior is never
...
LZGError lzg_fbg_posterior(const LZGFbg *prior,
                            ...
                            LZGFbg **out) {
    if (!prior || !out)
        return LZG_FAIL(LZG_ERR_NULL_ARG, "fbg_posterior: NULL argument");
    if (kappa < 0.0)
        return LZG_FAIL(LZG_ERR_PARAM_OUT_OF_RANGE,
                        "fbg_posterior: kappa must be >= 0, got %g", kappa);
    if (n_seqs > 0 && !sequences)
        return LZG_FAIL(LZG_ERR_NULL_ARG, "fbg_posterior: sequences NULL");
    ...
    if (!ind_per_rule || !ind_per_nt) {
        ...
        return LZG_FAIL(LZG_ERR_ALLOC, "fbg_posterior: alloc");
    }
    ...
    for (uint32_t s = 0; s < n_seqs; s++) {
        ...
        double w = lzg_fbg_abund_weight(prior->abundance_mode, a);
        if (w <= 0.0) continue;

        uint32_t n_steps = 0;
        LZGError err = lzg_fbg_decompose(prior, seq, len, steps, &n_steps);
        if (err != LZG_OK) continue;  /* out-of-alphabet seq → skip */

        for (uint32_t t = 0; t < n_steps; t++) {
            int32_t nt_idx = lzg_fbg_find_nt_for_step(prior, &steps[t]);
            if (nt_idx < 0) continue;
            int32_t rel = lzg_fbg_find_rule_in_nt(prior, (uint32_t)nt_idx, &steps[t]);
            if (rel < 0) continue;
    ...
    LZGFbg *post = lzg_fbg_new();
    if (!post) {
        free(ind_per_rule); free(ind_per_nt);
        return LZG_FAIL(LZG_ERR_ALLOC, "fbg_posterior: grammar alloc");
    }

    memcpy(post->char_to_idx, prior->char_to_idx, 256);
    memcpy(post->idx_to_char, prior->idx_to_char, LZG_FBG_ALPHABET_SIZE);
    ...
    post->nts = (LZGFbgNT *)malloc((size_t)post->n_nts * sizeof(LZGFbgNT));
    post->rules = (LZGFbgRule *)malloc((size_t)post->n_rules * sizeof(LZGFbgRule));
    if (!post->nts || !post->rules) {
        lzg_fbg_destroy(post);
        free(ind_per_rule); free(ind_per_nt);
        return LZG_FAIL(LZG_ERR_ALLOC, "fbg_posterior: copy alloc");
    }
    memcpy(post->nts, prior->nts, (size_t)post->n_nts * sizeof(LZGFbgNT));
    memcpy(post->rules, prior->rules, (size_t)post->n_rules * sizeof(LZGFbgRule));

    /* Step 3: DM update per NT. */
    for (uint32_t i = 0; i < post->n_nts; i++) {
        LZGFbgNT *nt = &post->nts[i];
    ...
    if (prior->max_length > 0 && prior->length_counts) {
        ...
        if (!post->length_counts) {
            lzg_fbg_destroy(post);
            free(ind_per_rule); free(ind_per_nt);
            return LZG_FAIL(LZG_ERR_ALLOC, "fbg_posterior: length_counts alloc");
        }
    ...
    LZGError err = lzg_fbg_finalize(post);
    free(ind_per_rule); free(ind_per_nt);
    if (err != LZG_OK) { lzg_fbg_destroy(post); return err; }
