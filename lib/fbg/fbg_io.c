...
static LZGError write_all(FILE *fh, const void *data, size_t n,
    ...
    if (fwrite(data, 1, n, fh) != n)
        return LZG_FAIL(LZG_ERR_IO_WRITE, "fbg_save: write failed (%s)",
                        strerror(errno));
...
LZGError lzg_fbg_save(const LZGFbg *g, const char *path) {
    if (!g || !path || !path[0])
        return LZG_FAIL(LZG_ERR_NULL_ARG, "fbg_save: NULL argument");

    FILE *fh = fopen(path, "wb");
    if (!fh) return LZG_FAIL(LZG_ERR_IO_OPEN, "fbg_save: cannot open %s: %s",
                              path, strerror(errno));
    ...
    W(&g->alphabet_size, 1);
    W(pad3, 3);
    W(g->idx_to_char, LZG_FBG_ALPHABET_SIZE);
    W(pad2, 2);
    ...
    if (fwrite(&crc, 1, 4, fh) != 4) {
        err = LZG_FAIL(LZG_ERR_IO_WRITE, "fbg_save: trailing crc write failed");
        goto fail;
    }
...
static LZGError read_all(FILE *fh, void *buf, size_t n, uint32_t *running_crc) {
    ...
    if (fread(buf, 1, n, fh) != n)
        return LZG_FAIL(LZG_ERR_IO_READ, "fbg_load: read failed (%s)",
...
LZGError lzg_fbg_load(const char *path, LZGFbg **out) {
    if (!path || !out)
        return LZG_FAIL(LZG_ERR_NULL_ARG, "fbg_load: NULL argument");
    *out = NULL;

    FILE *fh = fopen(path, "rb");
    if (!fh) return LZG_FAIL(LZG_ERR_IO_OPEN, "fbg_load: cannot open %s: %s",
                              path, strerror(errno));
    ...
    if (magic != FBG_MAGIC) {
        err = LZG_FAIL(LZG_ERR_IO_CORRUPT,
                       "fbg_load: bad magic 0x%08X (expected 0x%08X)",
                       magic, FBG_MAGIC);
        goto fail;
    }
    if (version != FBG_FORMAT_VER) {
        err = LZG_FAIL(LZG_ERR_IO_VERSION,
                       "fbg_load: format version %u unsupported (want %u)",
    ...
    R(&alphabet_size, 1);
    R(pad3, 3);
    R(idx_to_char, LZG_FBG_ALPHABET_SIZE);
    R(pad2, 2);

    if (alphabet_size > LZG_FBG_ALPHABET_SIZE) {
        err = LZG_FAIL(LZG_ERR_IO_CORRUPT,
                       "fbg_load: alphabet_size=%u exceeds cap %u",
                       alphabet_size, LZG_FBG_ALPHABET_SIZE);
        goto fail;
    }
    ...
    if (!g) { err = LZG_FAIL(LZG_ERR_ALLOC, "fbg_load: new"); goto fail; }

    /* Rebuild alphabet. */
    memset(g->char_to_idx, LZG_FBG_CHAR_UNKNOWN, 256);
    ...
    g->abundance_mode = (LZGFbgAbundanceMode)mode;
    g->smoothing = smoothing;
    g->backoff = (LZGFbgBackoff)backoff;

    g->nts = (LZGFbgNT *)calloc(n_nts, sizeof(LZGFbgNT));
    g->rules = (LZGFbgRule *)calloc(n_rules, sizeof(LZGFbgRule));
    if (!g->nts || !g->rules) {
        lzg_fbg_destroy(g);
        err = LZG_FAIL(LZG_ERR_ALLOC, "fbg_load: nts/rules");
        goto fail;
    }
    ...
    for (uint32_t i = 0; i < n_nts; i++) {
        ...
        if (err != LZG_OK) { lzg_fbg_destroy(g); goto fail; }
        g->nts[i].a           = d.a;
        g->nts[i].z           = d.z;
    ...
    if (max_length > 0) {
        ...
        if (!g->length_counts) {
            lzg_fbg_destroy(g);
            err = LZG_FAIL(LZG_ERR_ALLOC, "fbg_load: length_counts");
            goto fail;
        }
    ...
    if (fread(&expected_crc, 1, 4, fh) != 4) {
        lzg_fbg_destroy(g);
        err = LZG_FAIL(LZG_ERR_IO_READ, "fbg_load: CRC read failed");
        goto fail;
    }
    if (expected_crc != crc) {
        lzg_fbg_destroy(g);
        err = LZG_FAIL(LZG_ERR_IO_CORRUPT,
                       "fbg_load: CRC mismatch (file=0x%08X, computed=0x%08X)",
                       expected_crc, crc);
        goto fail;
    }
    ...
    LZGError ferr = lzg_fbg_finalize(g);
    if (ferr != LZG_OK) { lzg_fbg_destroy(g); return ferr; }
