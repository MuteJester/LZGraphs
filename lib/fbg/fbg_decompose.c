 ...
 * interned strings we don't want. Here we emit LZGFbgStep structs
 * directly, mapping chars to the grammar's alphabet index space.
 *
 * Reference: .private/flashback_grammar_2026-04-22/fbg_prototype.py
...
void lzg_fbg_alphabet_init(uint8_t char_to_idx[256],
    ...
    memset(char_to_idx, LZG_FBG_CHAR_UNKNOWN, 256);
    char_to_idx[(uint8_t)LZG_START_SENTINEL] = LZG_FBG_IDX_AT;
    char_to_idx[(uint8_t)LZG_END_SENTINEL]   = LZG_FBG_IDX_DOLL;
    idx_to_char[LZG_FBG_IDX_AT]   = (uint8_t)LZG_START_SENTINEL;
    idx_to_char[LZG_FBG_IDX_DOLL] = (uint8_t)LZG_END_SENTINEL;
    *alphabet_size = 2;
}
...
static LZGError decompose_rec(const uint8_t char_to_idx[256],
                              ...
                              bool is_root,
                              LZGFbgStep *out, uint32_t *n_out, uint32_t cap) {
    uint32_t span = hi - lo;
    ...
    if (a_idx == LZG_FBG_CHAR_UNKNOWN || z_idx == LZG_FBG_CHAR_UNKNOWN)
        return LZG_FAIL(LZG_ERR_INVALID_SEQUENCE,
                        "fbg decompose: char out of alphabet at [%u,%u)", lo, hi);

    /* Case 1: single char */
    if (span == 1) {
        LZGFbgStep *s = &out[(*n_out)++];
        s->kind       = LZG_FBG_RULE_LEAF_SINGLE;
        s->a_char     = a_idx;
        s->z_char     = a_idx;
        s->a_run_len  = 1;
        s->z_run_len  = 0;
    ...
    if (dst_a == LZG_FBG_CHAR_UNKNOWN || dst_z == LZG_FBG_CHAR_UNKNOWN)
        return LZG_FAIL(LZG_ERR_INVALID_SEQUENCE,
...
LZGError lzg_fbg_decompose(const LZGFbg *g,
    ...
    if (!g || !seq || !out || !n_out)
        return LZG_FAIL(LZG_ERR_NULL_ARG, "fbg_decompose: NULL argument");
    if (len == 0)
        return LZG_FAIL(LZG_ERR_EMPTY_INPUT, "fbg_decompose: empty sequence");
    ...
    if ((size_t)(wlen + 1) <= sizeof(stack_buf)) {
        ...
        if (!wrap) return LZG_FAIL(LZG_ERR_ALLOC, "fbg_decompose: alloc");
        heap = true;
    }
...
static LZGError reverse_rec(const uint8_t idx_to_char[LZG_FBG_ALPHABET_SIZE],
...
LZGError lzg_fbg_tree_to_string(const LZGFbg *g,
    ...
    if (!g || !steps || !out || !out_len)
        return LZG_FAIL(LZG_ERR_NULL_ARG, "fbg_tree_to_string: NULL argument");
    if (n == 0) {
        if (cap == 0)
            return LZG_FAIL(LZG_ERR_OVERFLOW, "fbg reverse: buffer too small");
        out[0] = '\0';
...
int32_t lzg_fbg_find_nt_for_step(const LZGFbg *g, const LZGFbgStep *step) {
    ...
    if (a >= LZG_FBG_ALPHABET_SIZE || z >= LZG_FBG_ALPHABET_SIZE) return -1;
    return g->nt_by_az[(uint32_t)a * LZG_FBG_ALPHABET_SIZE + (uint32_t)z];
}
