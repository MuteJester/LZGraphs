...
static void build_T_pow(const LZGFbg *g, double alpha, double *T) {
    uint32_t n = g->n_nts;
...
static void build_leaf_pow(const LZGFbg *g, double alpha, double *l) {
    uint32_t n = g->n_nts;
...
static void build_local_entropy(const LZGFbg *g, double *h) {
 ...
 * and non-S at 1..n-1.
 */
static LZGError length_polynomial(const LZGFbg *g, uint32_t L_max,
...
LZGError lzg_fbg_path_count_series(const LZGFbg *g, uint32_t L_max, double *out) {
    if (!g || !out) return LZG_FAIL(LZG_ERR_NULL_ARG, "fbg_path_count_series");
...
LZGError lzg_fbg_length_distribution(const LZGFbg *g, uint32_t L_max, double *out) {
    if (!g || !out) return LZG_FAIL(LZG_ERR_NULL_ARG, "fbg_length_distribution");
...
LZGError lzg_fbg_entropy(const LZGFbg *g, double *H_nats) {
    if (!g || !H_nats) return LZG_FAIL(LZG_ERR_NULL_ARG, "fbg_entropy");
    uint32_t n = g->n_nts;
    if (n == 0) return LZG_FAIL(LZG_ERR_EMPTY_INPUT, "fbg_entropy: empty");

    double *T = (double *)malloc((size_t)n * n * sizeof(double));
    double *h = (double *)malloc((size_t)n * sizeof(double));
    if (!T || !h) { free(T); free(h); return LZG_FAIL(LZG_ERR_ALLOC, "fbg_entropy"); }
...
LZGError lzg_fbg_effective_diversity(const LZGFbg *g, LZGEffectiveDiversity *out) {
    if (!g || !out) return LZG_FAIL(LZG_ERR_NULL_ARG, "fbg_effective_diversity");
    ...
    if (pc) {
        err = lzg_fbg_path_count_series(g, L_cap, pc);
        if (err == LZG_OK) {
            double total = 0.0;
...
LZGError lzg_fbg_power_sum(const LZGFbg *g, double alpha, double *out) {
    if (!g || !out) return LZG_FAIL(LZG_ERR_NULL_ARG, "fbg_power_sum");
    ...
    if (!T_a || !l_a) { free(T_a); free(l_a); return LZG_FAIL(LZG_ERR_ALLOC, "fbg_power_sum"); }
    ...
    double rho = lzg_fbg_spectral_radius_dense(T_a, n);
    if (rho >= 1.0 - 1e-9) {
        free(T_a); free(l_a);
        *out = INFINITY;
        LZG_WARN("fbg_power_sum: shadow spectral radius ρ(T_α)=%.6f ≥ 1 at "
    ...
    if (!(v > 0.0) || !isfinite(v)) {
        v = INFINITY;
        LZG_WARN("fbg_power_sum: solve gave non-positive %.4g at α=%.4f; "
...
LZGError lzg_fbg_hill_number(const LZGFbg *g, double alpha, double *out) {
    if (!g || !out) return LZG_FAIL(LZG_ERR_NULL_ARG, "fbg_hill_number");
    if (fabs(alpha - 1.0) < 1e-15) {
        double H;
        LZGError err = lzg_fbg_entropy(g, &H);
        if (err != LZG_OK) return err;
    ...
    double M_a;
    LZGError err = lzg_fbg_power_sum(g, alpha, &M_a);
    if (err != LZG_OK) return err;
...
LZGError lzg_fbg_hill_numbers(const LZGFbg *g,
                              ...
                              double *out) {
    if (!g || !alphas || !out)
        return LZG_FAIL(LZG_ERR_NULL_ARG, "fbg_hill_numbers");
    for (uint32_t i = 0; i < n; i++) {
        LZGError err = lzg_fbg_hill_number(g, alphas[i], &out[i]);
        if (err != LZG_OK) return err;
    }
    return LZG_OK;
}
