 ...
 *
 * Reference: flashback_grammar_prototype.py `__init__` + `_finalize`.
 */
#include <errno.h>
#include <stdio.h>
#include <stdlib.h>
...
#include "../graph_core/graph_build_ingest_internal.h"   /* stream helpers (reused)   */

/* ── Lifecycle ───────────────────────────────────────────────── */

LZGFlashbackGrammar *lzg_flashback_grammar_new(void) {
    LZGFlashbackGrammar *g = (LZGFlashbackGrammar *)calloc(1, sizeof(LZGFlashbackGrammar));
    if (!g) return NULL;
    lzg_flashback_grammar_alphabet_init(g->char_to_idx, g->idx_to_char, &g->alphabet_size);
    return g;
}
 ...
 * flattened into the final LZGFlashbackGrammar.rules[] slab at finalize.
 */
typedef struct {
...
typedef struct {
    ...
    int32_t      nt_index[LZG_FLASHBACK_GRAMMAR_ALPHABET_SIZE][LZG_FLASHBACK_GRAMMAR_ALPHABET_SIZE];
...
static LZGError build_state_ensure_nt(BuildState *bs, bool is_start,
    ...
    if ((uint32_t)idx >= bs->n_nts) {
        ...
        nt->cap = 8;
        nt->rules = (LZGFlashbackGrammarRule *)calloc(nt->cap, sizeof(LZGFlashbackGrammarRule));
...
static LZGError build_state_record_step(BuildState *bs,
    ...
    uint8_t src_a, src_z;
    if (step->is_start) {
        src_a = LZG_FLASHBACK_GRAMMAR_IDX_AT;
        src_z = LZG_FLASHBACK_GRAMMAR_IDX_DOLL;
    } else {
        src_a = step->a_char;
        src_z = step->z_char;
    }
    ...
    uint64_t key = lzg_flashback_grammar_rule_local_key(step->kind,
    ...
    if (inserted) {
        if (nt->count >= nt->cap) {
            ...
            LZGFlashbackGrammarRule *tmp = (LZGFlashbackGrammarRule *)realloc(
                nt->rules, (size_t)new_cap * sizeof(LZGFlashbackGrammarRule));
            if (!tmp) return LZG_FAIL(LZG_ERR_ALLOC, "fbg build: rules realloc");
            nt->rules = tmp;
            nt->cap = new_cap;
        }
        LZGFlashbackGrammarRule *r = &nt->rules[nt->count];
        memset(r, 0, sizeof(*r));
        r->kind       = step->kind;
     ...
     * LZGFlashbackGrammarRule.count as bits of a double. That's ugly; use a side-buffer
...
static LZGError finalize_from_build_state(LZGFlashbackGrammar *g, BuildState *bs,
...
LZGError lzg_flashback_grammar_build(LZGFlashbackGrammar *g,
                       ...
                       double smoothing,
                       LZGFlashbackGrammarBackoff backoff) {
    if (!g || !sequences) return LZG_FAIL(LZG_ERR_NULL_ARG, "flashback_grammar_build: NULL");
    if (n == 0) return LZG_FAIL(LZG_ERR_EMPTY_INPUT, "flashback_grammar_build: no sequences");
    if (smoothing < 0.0)
        return LZG_FAIL(LZG_ERR_PARAM_OUT_OF_RANGE, "flashback_grammar_build: smoothing < 0");
    ...
    for (uint32_t s = 0; s < n; s++) {
        ...
        for (const char *p = seq; *p; p++) {
            LZGError err = lzg_flashback_grammar_alphabet_observe(
    ...
    if (!bs.length_counts) {
        build_state_free(&bs);
        return LZG_FAIL(LZG_ERR_ALLOC, "flashback_grammar_build: length_counts alloc");
    }
    ...
    for (uint32_t s = 0; s < n; s++) {
        ...
        double w = lzg_flashback_grammar_abund_weight(mode, abund);
        if (w <= 0.0) continue;

        uint32_t n_steps = 0;
        LZGError err = lzg_flashback_grammar_decompose(g, seq, seq_len, steps, &n_steps);
        if (err != LZG_OK) {
            build_state_free(&bs);
            return err;
        }
        ...
        if (seq_len >= bs.length_cap) {
            ...
            if (!tmp) {
                build_state_free(&bs);
                return LZG_FAIL(LZG_ERR_ALLOC, "flashback_grammar_build: length_counts realloc");
            }
...
static LZGError bs_ensure_length_cap(BuildState *bs, uint32_t seq_len) {
    ...
    uint64_t *tmp = (uint64_t *)realloc(bs->length_counts,
                                         (size_t)new_cap * sizeof(uint64_t));
    if (!tmp) return LZG_FAIL(LZG_ERR_ALLOC, "flashback_grammar_build_file: length_counts");
    memset(tmp + bs->length_cap, 0,
           (size_t)(new_cap - bs->length_cap) * sizeof(uint64_t));
    bs->length_counts = tmp;
    bs->length_cap = new_cap;
...
LZGError lzg_flashback_grammar_build_file(LZGFlashbackGrammar *g, const char *path,
                             ...
                             LZGFlashbackGrammarBackoff backoff) {
    if (!g || !path || !path[0])
        return LZG_FAIL(LZG_ERR_NULL_ARG, "flashback_grammar_build_file: NULL");
    if (smoothing < 0.0)
        return LZG_FAIL(LZG_ERR_PARAM_OUT_OF_RANGE,
                        "flashback_grammar_build_file: smoothing < 0");

    FILE *fh = fopen(path, "r");
    if (!fh) return LZG_FAIL(LZG_ERR_IO_OPEN,
                              "flashback_grammar_build_file: cannot open '%s': %s",
                              path, strerror(errno));
    ...
    if (!bs.length_counts) {
        build_state_free(&bs);
        fclose(fh);
        return LZG_FAIL(LZG_ERR_ALLOC, "flashback_grammar_build_file: init");
    }
    ...
    while ((nread = lzg_getline_portable(&line, &line_cap, fh)) != -1) {
        ...
        double w = lzg_flashback_grammar_abund_weight(mode, count);
        if (w <= 0.0) continue;
        ...
        for (uint32_t i = 0; i < seq_len; i++) {
            LZGError aerr = lzg_flashback_grammar_alphabet_observe(
        ...
        uint32_t n_steps = 0;
        LZGError derr = lzg_flashback_grammar_decompose(g, seq, seq_len, steps, &n_steps);
...
double lzg_flashback_grammar_spectral_radius_dense(const double *T, uint32_t n) {
...
static LZGError finalize_from_build_state(LZGFlashbackGrammar *g, BuildState *bs,
    ...
    g->rules = (LZGFlashbackGrammarRule *)calloc(n_rules_total, sizeof(LZGFlashbackGrammarRule));
    if (!g->nts || !g->rules) {
    ...
    for (uint32_t i = 0; i < n_nts; i++) {
        ...
        LZGFlashbackGrammarNT *dst = &g->nts[i];
        dst->a = src->a;
        dst->z = src->z;
        ...
        for (uint32_t r = 0; r < src->count; r++) {
            LZGFlashbackGrammarRule *rule = &g->rules[rule_cursor];
            *rule = src->rules[r];
            ...
            if (rule->kind == LZG_FLASHBACK_GRAMMAR_RULE_INTERNAL) n_internal++;
            rule_cursor++;
        }
    }
    ...
    return lzg_flashback_grammar_finalize(g);
}

/* The shared finalize entry point — also called by posterior / subtract after
 * they populate g->rules and g->nts directly. */
LZGError lzg_flashback_grammar_finalize(LZGFlashbackGrammar *g) {
    if (!g) return LZG_FAIL(LZG_ERR_NULL_ARG, "flashback_grammar_finalize: NULL");
    if (g->n_nts == 0) return LZG_FAIL(LZG_ERR_EMPTY_INPUT, "flashback_grammar_finalize: n_nts=0");

    /* Build (a, z) → nt-index lookup. Also persisted on g->nt_by_az. */
    for (uint32_t k = 0; k < LZG_FLASHBACK_GRAMMAR_ALPHABET_SIZE * LZG_FLASHBACK_GRAMMAR_ALPHABET_SIZE; k++)
        g->nt_by_az[k] = -1;
    int32_t s_idx = -1;
    for (uint32_t i = 0; i < g->n_nts; i++) {
        if (g->nts[i].is_start) {
            ...
            uint32_t key = (uint32_t)g->nts[i].a * LZG_FLASHBACK_GRAMMAR_ALPHABET_SIZE
                         + (uint32_t)g->nts[i].z;
    ...
    #define IDX_OF(a, z) g->nt_by_az[(uint32_t)(a) * LZG_FLASHBACK_GRAMMAR_ALPHABET_SIZE + (uint32_t)(z)]

    /* Transition matrix T[n_nts × n_nts]: T[i][j] = Σ p(internal r: Mi → ... Mj). */
    uint32_t n = g->n_nts;
    double *T = (double *)calloc((size_t)n * n, sizeof(double));
    if (!T) return LZG_FAIL(LZG_ERR_ALLOC, "flashback_grammar_finalize: T alloc");
    for (uint32_t i = 0; i < n; i++) {
    ...
    if (g->backoff == LZG_FLASHBACK_GRAMMAR_BACKOFF_GT) {
        /* δ_nt = (# rules with count ≤ 1) / total_count, capped at 0.5. */
        for (uint32_t i = 0; i < g->n_nts; i++) {
            LZGFlashbackGrammarNT *nt = &g->nts[i];
        ...
        if (!g->rule_marginal)
            return LZG_FAIL(LZG_ERR_ALLOC, "flashback_grammar_finalize: marginal alloc");
