 ...
 *
 * Reference: flashback_grammar_prototype.py `without`.
 *
...
static inline double recover_frac_count(const LZGFlashbackGrammar *g,
                                         const LZGFlashbackGrammarNT *nt,
                                         const LZGFlashbackGrammarRule *rule) {
...
LZGError lzg_flashback_grammar_subtract(const LZGFlashbackGrammar *g,
    ...
    if (!g || !out)
        return LZG_FAIL(LZG_ERR_NULL_ARG, "flashback_grammar_subtract: NULL argument");
    if (n_seqs > 0 && !sequences)
        return LZG_FAIL(LZG_ERR_NULL_ARG, "flashback_grammar_subtract: sequences NULL");

    /* Step 1: accumulate subtraction amounts per absolute rule index. */
    double *sub = (double *)calloc(g->n_rules, sizeof(double));
    if (!sub) return LZG_FAIL(LZG_ERR_ALLOC, "flashback_grammar_subtract: sub alloc");
    ...
    for (uint32_t s = 0; s < n_seqs; s++) {
        ...
        double w = lzg_flashback_grammar_abund_weight(g->abundance_mode, a);
        if (w <= 0.0) continue;
        ...
        for (uint32_t t = 0; t < n_steps; t++) {
            int32_t nt_idx = lzg_flashback_grammar_find_nt_for_step(g, &steps[t]);
            if (nt_idx < 0) continue;
            int32_t rel = lzg_flashback_grammar_find_rule_in_nt(g, (uint32_t)nt_idx, &steps[t]);
            if (rel < 0) continue;
    ...
    if (!surv || !keep_rule || !keep_nt) {
        ...
        return LZG_FAIL(LZG_ERR_ALLOC, "flashback_grammar_subtract: scratch alloc");
    }
    ...
    if (n_keep_nts == 0) {
        ...
        return LZG_FAIL(LZG_ERR_EMPTY_INPUT,
                        "flashback_grammar_subtract: would leave an empty grammar");
    }
    if (!keep_nt[g->start_nt]) {
        free(surv); free(keep_rule); free(keep_nt);
        return LZG_FAIL(LZG_ERR_EMPTY_INPUT,
                        "flashback_grammar_subtract: would remove the start non-terminal");
    }
    ...
    if (!old_to_new) {
        free(surv); free(keep_rule); free(keep_nt);
        return LZG_FAIL(LZG_ERR_ALLOC, "flashback_grammar_subtract: map alloc");
    }
    ...
    LZGFlashbackGrammar *sub_g = lzg_flashback_grammar_new();
    if (!sub_g) {
        free(surv); free(keep_rule); free(keep_nt); free(old_to_new);
        return LZG_FAIL(LZG_ERR_ALLOC, "flashback_grammar_subtract: grammar alloc");
    }

    memcpy(sub_g->char_to_idx, g->char_to_idx, 256);
    memcpy(sub_g->idx_to_char, g->idx_to_char, LZG_FLASHBACK_GRAMMAR_ALPHABET_SIZE);
    sub_g->alphabet_size = g->alphabet_size;
    sub_g->n_nts = n_keep_nts;
    sub_g->start_nt = 0;
    sub_g->n_rules = n_keep_rules;

    sub_g->nts = (LZGFlashbackGrammarNT *)calloc(n_keep_nts, sizeof(LZGFlashbackGrammarNT));
    sub_g->rules = (LZGFlashbackGrammarRule *)calloc(n_keep_rules, sizeof(LZGFlashbackGrammarRule));
    if (!sub_g->nts || !sub_g->rules) {
        lzg_flashback_grammar_destroy(sub_g);
        free(surv); free(keep_rule); free(keep_nt); free(old_to_new);
        return LZG_FAIL(LZG_ERR_ALLOC, "flashback_grammar_subtract: nts/rules alloc");
    }
    ...
    for (uint32_t new_i = 0; new_i < n_keep_nts; new_i++) {
        ...
        const LZGFlashbackGrammarNT *old_nt = &g->nts[old_i];
        LZGFlashbackGrammarNT *new_nt = &sub_g->nts[new_i];
        *new_nt = *old_nt;
        ...
        for (uint32_t r = 0; r < old_nt->n_rules; r++) {
            ...
            LZGFlashbackGrammarRule *new_rule = &sub_g->rules[rule_cursor];
            *new_rule = g->rules[abs_old];
            /* Stash fractional count in weight for now; MLE below normalises. */
            new_rule->weight = surv[abs_old];
            total += surv[abs_old];
            if (new_rule->kind == LZG_FLASHBACK_GRAMMAR_RULE_INTERNAL) n_internal++;
            rule_cursor++;
            new_nt->n_rules++;
        }
    ...
    for (uint32_t i = 0; i < sub_g->n_nts; i++) {
        LZGFlashbackGrammarNT *nt = &sub_g->nts[i];
        double denom = nt->total_count + g->smoothing * (double)nt->n_rules;
        for (uint32_t r = 0; r < nt->n_rules; r++) {
            uint32_t abs_idx = nt->rule_offset + r;
    ...
    if (g->max_length > 0 && g->length_counts) {
        ...
        if (!sub_g->length_counts) {
            lzg_flashback_grammar_destroy(sub_g);
            free(surv); free(keep_rule); free(keep_nt); free(old_to_new);
            return LZG_FAIL(LZG_ERR_ALLOC, "flashback_grammar_subtract: length_counts");
        }
    ...
    LZGError err = lzg_flashback_grammar_finalize(sub_g);
    if (err != LZG_OK) { lzg_flashback_grammar_destroy(sub_g); return err; }
