 ...
 *
 * Reference: flashback_grammar_prototype.py `sample` and `top_k_sequences`.
 */
#include <stdlib.h>
#include <string.h>
...
static uint32_t sample_rule_at(const LZGFlashbackGrammar *g, uint32_t nt_idx, LZGRng *rng) {
...
static LZGError sample_rec(const LZGFlashbackGrammar *g, uint32_t nt_idx,
                            LZGRng *rng,
                            LZGFlashbackGrammarStep *steps, uint32_t *n_steps,
                            double *log_p) {
    if (*n_steps >= LZG_FLASHBACK_GRAMMAR_MAX_STEPS)
    ...
    const LZGFlashbackGrammarNT *nt = &g->nts[nt_idx];
    const LZGFlashbackGrammarRule *rule = &g->rules[nt->rule_offset + rel];

    double w = rule->weight;
    if (w > 0.0) *log_p += log(w);
    else         *log_p += LZG_LOG_EPS;

    LZGFlashbackGrammarStep *s = &steps[(*n_steps)++];
    s->kind       = rule->kind;
...
LZGError lzg_flashback_grammar_simulate(const LZGFlashbackGrammar *g, uint32_t n,
    ...
    if (!g || !rng || !out)
        return LZG_FAIL(LZG_ERR_NULL_ARG, "flashback_grammar_simulate: NULL");
    if (g->n_nts == 0)
        return LZG_FAIL(LZG_ERR_NOT_BUILT, "flashback_grammar_simulate: empty grammar");
    ...
    for (uint32_t i = 0; i < n; i++) {
        ...
        char buf[2048];
        uint32_t out_len = 0;
        err = lzg_flashback_grammar_tree_to_string(g, steps, n_steps, buf, sizeof(buf), &out_len);
        if (err != LZG_OK) {
            out[i].sequence = strdup("");
            out[i].seq_len  = 0;
...
LZGError lzg_flashback_grammar_dynamic_range(const LZGFlashbackGrammar *g, uint32_t L_max,
                                LZGDynamicRange *out) {
    if (!g || !out)
        return LZG_FAIL(LZG_ERR_NULL_ARG, "flashback_grammar_dynamic_range: NULL");
    ...
    if (!max_lp || !min_lp) {
        ...
        return LZG_FAIL(LZG_ERR_ALLOC, "flashback_grammar_dynamic_range: alloc");
    }
...
static LZGError reconstruct(const LZGFlashbackGrammar *g, const KBest *kb,
    ...
    const LZGFlashbackGrammarNT *nt = &g->nts[nt_idx];
    const LZGFlashbackGrammarRule *rule = &g->rules[nt->rule_offset + e->rule_idx];
...
LZGError lzg_flashback_grammar_top_k_sequences(const LZGFlashbackGrammar *g, uint32_t K,
    ...
    if (!g || !out || !n_out || K == 0)
        return LZG_FAIL(LZG_ERR_NULL_ARG, "flashback_grammar_top_k: bad args");
    ...
    if (!kb || !counts) {
        free(kb); free(counts);
        return LZG_FAIL(LZG_ERR_ALLOC, "flashback_grammar_top_k: alloc");
    }
    ...
    if (!final || !from_L || !from_k) {
        ...
        free(kb); free(counts);
        return LZG_FAIL(LZG_ERR_ALLOC, "flashback_grammar_top_k: final alloc");
    }
