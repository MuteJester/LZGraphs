/**
 * @file flashback_grammar_internal.h
 * @brief Shared helpers for the FlashBackGrammar implementation.
 *
 * Not part of the public API. Do not include from outside lib/flashback_grammar/.
 */
#ifndef LZG_FLASHBACK_GRAMMAR_INTERNAL_H
#define LZG_FLASHBACK_GRAMMAR_INTERNAL_H

#include "lzgraph/flashback_grammar.h"

#define LZG_FLASHBACK_GRAMMAR_CHAR_UNKNOWN 0xFFu

/* Reserved alphabet indices for sentinels. */
#define LZG_FLASHBACK_GRAMMAR_IDX_AT    0u   /* '@' */
#define LZG_FLASHBACK_GRAMMAR_IDX_DOLL  1u   /* '$' */
 ...
 */
static inline uint64_t lzg_flashback_grammar_rule_marginal_key(uint8_t kind,
 ...
 */
static inline uint64_t lzg_flashback_grammar_rule_local_key(uint8_t kind,
                                              uint8_t a_run_len, uint8_t z_run_len,
                                              uint8_t dst_a, uint8_t dst_z,
...
typedef struct {
    uint32_t table[LZG_FLASHBACK_GRAMMAR_ALPHABET_SIZE][LZG_FLASHBACK_GRAMMAR_ALPHABET_SIZE];
    uint32_t start_nt;  /* always 0 — S is the start NT */
} LZGFlashbackGrammarNTLookup;
...
static inline uint32_t lzg_flashback_grammar_nt_lookup_get(const LZGFlashbackGrammarNTLookup *lut,
                                              uint8_t a, uint8_t z) {
...
static inline double lzg_flashback_grammar_abund_weight(LZGFlashbackGrammarAbundanceMode mode, uint64_t a) {
    switch (mode) {
    case LZG_FLASHBACK_GRAMMAR_ABUND_NONE:   return 1.0;
    case LZG_FLASHBACK_GRAMMAR_ABUND_LINEAR: return (double)a;
    case LZG_FLASHBACK_GRAMMAR_ABUND_LOG:    return log1p((double)a);
    }
    return 1.0;
}
...
#endif /* LZG_FLASHBACK_GRAMMAR_INTERNAL_H */
