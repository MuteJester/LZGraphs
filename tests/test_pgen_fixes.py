...
1. Initial probability uses initial_states_probability (not subpattern_individual_probability)
...
class TestInitialProbabilityFix:
    """Verify walk_probability uses initial_states_probability, not subpattern_individual_probability."""

    def test_initial_prob_uses_initial_states(self, aap_lzgraph):
        """The initial probability should come from initial_states_probability."""
        sequence = "CASSLGQAYEQYF"
        walk = AAPLZGraph.encode_sequence(sequence)
        first_node = walk[0]

        # Verify the first node IS in initial_states_probability
        assert first_node in aap_lzgraph.initial_states_probability.index

        # The values should differ between the two distributions
        init_prob = aap_lzgraph.initial_states_probability[first_node]
        subp_prob = aap_lzgraph.subpattern_individual_probability.loc[first_node, 'proba']
    ...
    def test_unrecognized_initial_state_returns_zero(self, aap_lzgraph):
        """If the first node is not in initial_states_probability, return 0."""
    ...
    def test_unrecognized_initial_state_log_returns_neg_inf(self, aap_lzgraph):
        """If the first node is not in initial_states_probability, return -inf in log mode."""
        fake_walk = ['ZZZZ_99', 'YYYY_100']
...
class TestAllEdgesMissingFix:
    ...
    def test_all_edges_missing_returns_zero(self, aap_lzgraph):
        ...
        first_node = aap_lzgraph.initial_states_probability.index[0]
...
class TestTerminalStateFactor:
    ...
    def test_terminal_factor_included(self, aap_lzgraph):
        ...
        if last_node in aap_lzgraph.terminal_state_data.index:
            stop_prob = aap_lzgraph.terminal_state_data.loc[last_node, 'wsif/sep']
    ...
    def test_non_terminal_last_node_gets_epsilon(self, aap_lzgraph):
        ...
        terminal_nodes = set(aap_lzgraph.terminal_state_data.index)
...
class TestInitialStateThreshold:
    ...
    def test_threshold_zero_includes_all(self, test_data_aap):
        ...
        g_default = AAPLZGraph(test_data_aap, verbose=False, initial_state_threshold=5)
        g_zero = AAPLZGraph(test_data_aap, verbose=False, initial_state_threshold=0)

        # Zero threshold should include at least as many initial states
        assert len(g_zero.initial_states_probability) >= len(g_default.initial_states_probability)

    def test_high_threshold_excludes_rare(self, test_data_aap):
        """With high threshold, rare initial states should be excluded."""
        g_high = AAPLZGraph(test_data_aap, verbose=False, initial_state_threshold=100)
        g_default = AAPLZGraph(test_data_aap, verbose=False, initial_state_threshold=5)

        # High threshold should include fewer initial states
        assert len(g_high.initial_states_probability) <= len(g_default.initial_states_probability)
...
class TestNDPImputation:
    ...
    def test_ndp_missing_edge_imputed(self, ndp_lzgraph):
        ...
        first_node = ndp_lzgraph.initial_states_probability.index[0]
